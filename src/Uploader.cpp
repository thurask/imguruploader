...
{
    m_apiKey = QLatin1String("1be5303e68928e6688473262df7c9de7");
    m_manager = new QNetworkAccessManager(this);
    connect(m_manager, SIGNAL(finished(QNetworkReply *)),
            this, SLOT(requestFinished(QNetworkReply *)));
}

int Uploader::fileSize(QString fileName)
{
    QFile file(fileName);
    return file.size();
}
                ...
                while (!innerNode.isNull()) {
                    if (innerNode.isElement()) {
                        ...
                        if (e.tagName() == "original") {
                            linksMap.insert("original", e.text());
                        }
                        else if (e.tagName() == "small_square") {
                            linksMap.insert("small_square", e.text());
                        }
                        else if (e.tagName() == "large_thumbnail") {
                            linksMap.insert("large_thumbnail", e.text());
                        }
                        else if (e.tagName() == "imgur_page") {
                            linksMap.insert("imgur_page", e.text());
                        }
                        else if (e.tagName() == "delete_page") {
                            linksMap.insert("delete_page", e.text());
                        }
                    }
